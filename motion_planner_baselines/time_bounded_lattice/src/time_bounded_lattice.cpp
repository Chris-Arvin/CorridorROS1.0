#include "time_bounded_lattice/time_bounded_lattice.h"
#include <Eigen/Core>
#include <chrono>

namespace time_bounded_lattice
{
TimeBoundedLattice::TimeBoundedLattice(base_local_planner::CostmapModel* costmap_model,
  ...
  , dt_(0.2)
{
  ...
  sub_person = nh.subscribe<pedsim_msgs::TrackedPersons>("/pedsim_visualizer/tracked_persons", 1, boost::bind(&TimeBoundedLattice::PersonCallback, this ,_1));
}

TimeBoundedLattice::~TimeBoundedLattice()
{
}
 ...
 * */
void TimeBoundedLattice::initializeCandidateTargets(const double& forward_detect_latitude)
{
 ...
 * */
void TimeBoundedLattice::transformCandidateTargets(const Pose2D& robot_pose, const std::vector<Pose2D>& global_plan){
  Pose2D pose;
 ...
 * */
void TimeBoundedLattice::scorePaths(const int& size_x, const int& size_y, const double& resolution, const double& origin_x, const double& origin_y, const std::vector<Pose2D>& global_plan, unsigned char const* const* costmap){
  tracked_path_.clear();
 ...
 * */
double TimeBoundedLattice::scorePath(const std::vector<Pose2D>& path, const int& size_x, const int& size_y, const double& resolution, const double& origin_x, const double& origin_y, const std::vector<Pose2D>& global_plan, unsigned char const* const* costmap){
  // cost1: occupy
  int mx,my;
  int occupy = 0;
...
void TimeBoundedLattice::wolrd2map(const double& wx, const double& wy, int& mx, int& my, const double& resolution, const double& origin_x, const double& origin_y){
  mx = int((wx-origin_x)/resolution);
  my = int((wy-origin_y)/resolution);
}

void TimeBoundedLattice::fitAllPaths(const Pose2D& robot_pose){
...
void TimeBoundedLattice::PersonCallback(const pedsim_msgs::TrackedPersons::ConstPtr& persons) {
  ...
  for (auto person:persons->tracks){
    ...
    for (int t=0; t<=100; t++){
      temp_trajectory.push_back(Pose2D(temp_px+temp_vx*t*dt_, temp_py+temp_vy*t*dt_, 0));
    }
    predicted_trajectories_.push_back(temp_trajectory);
  }
}

bool TimeBoundedLattice::isCollisionWithPerons(const std::vector<Pose2D>& path){
  int min_size = path.size();
  double safe_dis = 0.2+0.4; //todo 改成从param接收
...
double TimeBoundedLattice::constrainTheta(double theta){
  while (theta > M_PI){
    theta -= 2*M_PI;
  }
  while (theta < -M_PI){
 ...
 * */
std::vector<Pose2D> TimeBoundedLattice::fitPath(const Pose2D& p0, const Pose2D& p3){
  // const double disperse_number = std::min(30.0,forward_detect_latitude_/robot_vel_.v/dt_);
...
Velocity TimeBoundedLattice::generateVel(const Pose2D& robot_pose){
...
bool TimeBoundedLattice::computeVelocityCommands(const Velocity& robot_vel, const Pose2D& robot_pose,
...
bool TimeBoundedLattice::isPathFeasible(const std::vector<Pose2D>& path)
{
...
void TimeBoundedLattice::publishBezierPaths(const std::vector<std::vector<Pose2D> >& bezier_paths,
...
void TimeBoundedLattice::publishBezierPath(const std::vector<Pose2D>& bezier_path, ros::Publisher publisher)
{
  nav_msgs::Path gui_path;
  gui_path.header.frame_id = cfg_->map_frame;
  gui_path.header.stamp = ros::Time::now();
