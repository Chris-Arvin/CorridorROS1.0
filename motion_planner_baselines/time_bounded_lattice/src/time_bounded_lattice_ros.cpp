...
#include "time_bounded_lattice/time_bounded_lattice_ros.h"

// register this planner as a BaseLocalPlanner plugin
PLUGINLIB_EXPORT_CLASS(time_bounded_lattice::TimeBoundedLatticeROS, nav_core::BaseLocalPlanner)

namespace time_bounded_lattice
{
TimeBoundedLatticeROS::TimeBoundedLatticeROS()
...
void TimeBoundedLatticeROS::freeMemory()
{
  if (charmap_)
  {
...
void TimeBoundedLatticeROS::allocateMemory()
{
...
void TimeBoundedLatticeROS::initialize(std::string name, tf2_ros::Buffer* tf, costmap_2d::Costmap2DROS* costmap_ros)
{
  ...
  if (!initialized_)
  {
    ...
    planner_ = new TimeBoundedLattice(costmap_model_, footprint_spec_, robot_inscribed_radius_,
    ...
    ROS_DEBUG("time_bounded_lattice_planner plugin initialized.");
  }
  else
  {
    ROS_WARN("time_bounded_lattice_planner has already been initialized, doing nothing.");
  }
}

bool TimeBoundedLatticeROS::setPlan(const std::vector<geometry_msgs::PoseStamped>& orig_global_plan)
{
  // check if plugin is initialized
  if (!initialized_)
  {
    ROS_ERROR("time_bounded_lattice_planner has not been initialized, please call initialize() before using this planner");
    return false;
  }
...
bool TimeBoundedLatticeROS::computeVelocityCommands(geometry_msgs::Twist& cmd_vel)
{
  // check if plugin is initialized
  if (!initialized_)
  {
...
bool TimeBoundedLatticeROS::isGoalReached()
{
  // check if plugin is initialized
  if (!initialized_)
  {
...
bool TimeBoundedLatticeROS::pruneGlobalPlan(const tf2_ros::Buffer& tf, const geometry_msgs::PoseStamped& global_pose,
...
bool TimeBoundedLatticeROS::transformGlobalPlan(
...
double TimeBoundedLatticeROS::estimateLocalGoalOrientation(const std::vector<geometry_msgs::PoseStamped>& global_plan,
...
void TimeBoundedLatticeROS::publishGlobalPlan(const std::vector<geometry_msgs::PoseStamped>& global_plan)
{
