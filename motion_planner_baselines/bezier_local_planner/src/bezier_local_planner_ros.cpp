...
#include "bezier_local_planner/bezier_local_planner_ros.h"

// register this planner as a BaseLocalPlanner plugin
PLUGINLIB_EXPORT_CLASS(bezier_local_planner::BezierLocalPlannerROS, nav_core::BaseLocalPlanner)

namespace bezier_local_planner
{
BezierLocalPlannerROS::BezierLocalPlannerROS()
...
void BezierLocalPlannerROS::freeMemory()
{
  if (charmap_)
  {
...
void BezierLocalPlannerROS::allocateMemory()
{
...
void BezierLocalPlannerROS::initialize(std::string name, tf2_ros::Buffer* tf, costmap_2d::Costmap2DROS* costmap_ros)
{
  ...
  if (!initialized_)
  {
    ...
    planner_ = new BezierLocalPlanner(costmap_model_, footprint_spec_, robot_inscribed_radius_,
...
bool BezierLocalPlannerROS::setPlan(const std::vector<geometry_msgs::PoseStamped>& orig_global_plan)
{
...
bool BezierLocalPlannerROS::computeVelocityCommands(geometry_msgs::Twist& cmd_vel)
{
  // check if plugin is initialized
  if (!initialized_)
  {
...
bool BezierLocalPlannerROS::isGoalReached()
{
  // check if plugin is initialized
  if (!initialized_)
  {
...
bool BezierLocalPlannerROS::pruneGlobalPlan(const tf2_ros::Buffer& tf, const geometry_msgs::PoseStamped& global_pose,
...
bool BezierLocalPlannerROS::transformGlobalPlan(
...
double BezierLocalPlannerROS::estimateLocalGoalOrientation(const std::vector<geometry_msgs::PoseStamped>& global_plan,
...
void BezierLocalPlannerROS::publishGlobalPlan(const std::vector<geometry_msgs::PoseStamped>& global_plan)
{
