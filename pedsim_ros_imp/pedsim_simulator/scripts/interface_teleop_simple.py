...
gazetopic_list=["/people{}/keyboard/gaze".format(i) for i in range(len(keyboard_list))]
...
if __name__=="__main__":
    ...
    if person_mode!=2 and robot_mode==2:
        n=1
        i=0
        env = InterfaceSystem(n+gaze_plus)
    ...
    # 行人和机器人都是手动控制
        ...
        while i<n:
            ...
            # control persons
                ...
                obj.set_gazepublisher(gazetopic_list[i-1])
                obj.draw_obj(location)
                unit_list.append(obj)
                i+=1
    
    if gaze_enable:
        location = (math.floor(n/MAX_LINE_NUM),math.ceil(n%MAX_LINE_NUM))
